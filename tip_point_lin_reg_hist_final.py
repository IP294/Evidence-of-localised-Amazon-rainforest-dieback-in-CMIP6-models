...
Kendall tau analysis of the temperature potential EWS
...
## initialise zero array for risk
risk = np.zeros((np.size(models),np.size(thresholds)))

######### define arrays in which final information will be stored  ############
...
for l in range (0,np.size(models)):
    ...
    #################### average weighted temperature across the globe ####################
    ...
    ########################### plot histogram #############################
    edges = thresholds
    ax[l].title.set_text(model)
    ax[l].hist([b0_2, b0_1, b0_3, b0_4, b0_n], weights = [(area2/am_area)*100, (area1/am_area)*100, + (area3/am_area)*100, (area4/am_area)*100, (arean/am_area)*100],
                stacked=True,color=['red','purple','purple','purple','purple'], label=["T<0, AS<0", "T>0, AS>0", "T<0, AS>0", "T>0, AS<0", "No Tip"], bins=edges)
    ax[l].text(0.88, 0.88, '('+alphabet[l]+')', transform=ax[l].transAxes)
    ax[l].set_xticks(np.arange(-0.5, 3.5, 1))
#%%
...
edges = thresholds
s = np.size(models)
ax[7].title.set_text('Compiled Models')
ax[7].hist([tip2, tip1, tip3, tip4, notip], weights = [(areafrac2/s)*100, (areafrac1/s)*100, (areafrac3/s)*100, (areafrac4/s)*100, (areafracn/s)*100], 
           stacked=True,color=['red','purple', 'purple','purple', 'purple'], bins = edges)
ax[7].text(0.88, 0.88, '('+alphabet[7]+')', transform=ax[7].transAxes)
ax[7].set_xticks(np.arange(-0.5, 3.5, 1))
...
for i in range (1, np.size(thresholds)):
    ...
    ## for red coded tipping points (type 2)
...
ax[7].legend(['Abrupt shift', 'No abrupt shift'], loc='lower center', bbox_to_anchor=(0.5, -0.6), ncol=2)    
ax[8].bar(thresholds-0.1, tip_count*100, width=0.2, bottom=None, align='center', color='darkred')    
...
ax[8].set_ylabel('Risk of an AS (%)')
ax[8].set_xlim([-0.6, 2.0])
ax[8].text(0.88, 0.88, '('+alphabet[8]+')', transform=ax[8].transAxes)
ax[8].set_xticks(np.arange(-0.5, 2.5, 0.5))
ax[8].set_yticks(np.arange(0,125,25))
plt.show()

# savepath = 'C:/Users/impy2/OneDrive/Documents/Uni Yr3/Tipping Points Project/Figures/Paper/Figure 4/Hist_experiments/'
# filename = savepath+'warming_seasonal_cycle_diff_V8.svg'
